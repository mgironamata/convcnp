...
filepath = r'../data/camels_processed/maurer_model_output.csv'
...
df = df[(df['YR'] >= s_year) & (df['MNTH'] >= s_month) & (df['DY'] <= e_year) & (df['MNTH'] <= e_month)]
...
gen = convcnp.data_hydro.HydroGenerator(s_year=s_year,
                                            s_month=s_month,
                                            s_day=s_day,
                                            e_year=e_year,
                                            e_month=e_month,
...
NUM_TEST_TASKS = 2048/4
#gen_test = convcnp.data.GPGenerator(kernel=kernel, num_tasks=NUM_TEST_TASKS)

gen_test = convcnp.data_hydro.HydroGenerator(s_year=s_year,
                                            s_month=s_month,
                                            s_day=s_day,
                                            e_year=e_year,
                                            e_month=e_month,
